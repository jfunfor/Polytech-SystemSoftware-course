...
- 1. Заголовок TCP добавляется к данным приложения.
- 2. Заголовок содержит необходимую информацию для управления соединением.
- 3. Сегмент передается по сети к получателю, где он разбирается и передается на уровень приложения.
...
TCP использует трехступенчатый процесс установки соединения, называемый "three-way handshake":
  ...
  - 2. Сервер отвечает с подтверждением (SYN-ACK).
  - 3. Клиент отправляет окончательное подтверждение (ACK), после чего соединение устанавливается.
...
- 1. Измерение состояния сети: TCP использует различные методы для оценки состояния сети, включая задержку и потерю пакетов. Это позволяет определить, насколько сеть загружена в текущий момент.
- 2. Регулировка скорости передачи: на основе измерений состояния сети TCP регулирует скорость передачи данных. Если сеть перегружена (например, наблюдаются потери пакетов), TCP снижает скорость отправки данных.
